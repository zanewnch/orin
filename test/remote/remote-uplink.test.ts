...
import { REMOTE_PROTO_VERSION } from "../../src/remote/remote-frames";
...
describe("RemoteUplink socket-level project authorization", () => {
  ...
  it("source gate: deliver/broadcastTo ownership filter is on the write path", () => {
    const src = fs.readFileSync(
      path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "..", "src", "remote", "remote-uplink.ts"),
      "utf8",
    );
