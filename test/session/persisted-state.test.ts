...
import { PersistedState, DISK_KEYS, type StateFs, type MementoLike } from "../../src/session/persisted-state";
...
describe("PersistedState", () => {
  ...
  it("serializes grok.mcpConnectors as ids and endpoints, never a key", async () => {
    const { connectConnector } = await import("../../src/mcp/mcp-connectors");
    const { state, fs, memento } = make();
