...
import {
  ...
  CLI_UPDATE_VERSION_KEY,
  EXTRA_PROJECT_FOLDERS_KEY,
  IMPLICIT_CHIP_HIDDEN_KEY,
  INSTALL_ID_KEY,
  OAUTH_SHADOW_WARNING_KEY,
  PROJECT_PROVIDER_DEFAULTS_KEY,
...
import { GrokDiffContentProvider, GROK_DIFF_SCHEME } from "./diff";
import { guessMediaMime, hasLiveWork, NEEDS_YOU_KEEP_AWAKE_MS, statKindSafe } from "./helpers";
import { getNonce, renderChatHtml } from "./html";
import { isCanonicallyInsideRoot } from "../composer/file-tree";
import * as fs from "node:fs";
import * as os from "node:os";
import * as path from "node:path";
import { spawn } from "node:child_process";
import { AcpClient, EffortLevel, ExitPlanRequest, PermissionRequest, QuestionRequest } from "../acp/acp";
...
import { routinesMessageForRemote } from "../remote/remote-policy";
import { PersistedState } from "../session/persisted-state";
import {
  Session,
  SessionStartIntent,
  SessionStatus,
...
import { buildReapCandidates, selectReapable, computeDot, Dot } from "../session/session-pool";
import { resolveVoiceKey, extractGrokAuthKey, parseVoiceCommand, buildSttKeyterms, voiceSettingForRepo, voiceSettingWriteTarget, sanitizeVoiceSendPhrase, sanitizeVoiceKeyterms, voiceConfiguredFingerprint, DEFAULT_SEND_PHRASE, MAX_RECORDING_SECONDS } from "../voice/voice";
import { VoiceRecorder, transcribeAudio, resolveWindowsAudioDevice } from "../voice/voice-recorder";
import { PcmVoiceStreamer, VoiceStreamer } from "../voice/voice-streamer";
import { summarizeForSpeech } from "../voice/speech-summary";
import type { PromptResultMeta, PromptUsage, SessionInfoContext } from "../acp/acp-dispatch";
import { MediaRef, adapterCompactSignal, adapterContextOccupancy, agentTimestampMsFromMeta, autoCompactStartedNote, childStreamFromRoute, commandOutputForToolCall, commandOutputFromLiveTerminal, contextUsedFromCompactNotification, enforceCompleteSessionCost, errorDetail, gateZeroTokenMeta, isAuthErrorText, isCredentialError, isIncompatibleAgentError, isResumeNotFound, isRateLimitError, isSubagentLifecycleUpdate, occupancyFromAdapterTurn, parseSessionInfoContext, permissionOutcomeFor, promptErrorText, rateLimitNoticeText, sessionInfoCacheFresh, sumUsage, summarizeBackgroundCommand, usageIsRealMeasurement, type UpdateRoute } from "../acp/acp-dispatch";
import { createMcpPrepareState, prepareMcpToolCall } from "../mcp/mcp-tool";
import { modeToRemember, startsInYolo } from "../session/mode-prefs";
import { beginAuthRecovery, oauthShadowsXaiApiKey } from "../auth/auth-recovery";
import {
  WELCOME_TIPS_KEY,
...
import { commandOnPath, runGitClone } from "../projects/git-clone";
import {
  DISCONNECTED_GITHUB,
...
import { probeClaudeAuthStatus, runDeviceLogin, type DeviceLoginHandle } from "../auth/device-login-run";
import {
  GITHUB_CLI_BIN,
  githubDeviceLoginFailureText,
  isGithubCliMissing,
  runGithubDeviceLogin,
} from "../auth/github-device-login";
import {
  GITHUB_CLI_DOWNLOAD,
...
import { buildPromptWithImages, buildQueuedPromptWithImages, type PromptImageInput, type QueuedPromptContribution } from "../composer/prompt-builder";
import {
  chipsForQueueSend,
...
} from "../session/queued-send";

import { applyHostPlanSlashToItems, matchSlashCommand, mergeHostSlashCommands, parseHostPlanSlash } from "../composer/slash-filter";
import {
  MENTION_INDEX_LIMIT,
...
} from "../composer/file-upload";
import { MAX_DIFF_EXPAND_BYTES, expandDiffToWholeFile } from "../composer/diff-view";
import { applyAgentModeToHostPlan, effectivePlanActive, isPlanReviewPermission, permissionAnswerAllowed, permissionOptionsForPlan, pickRejectOption, planReviewVerdictForOption, planTextFromPermissionToolCall, shouldRejectPermission } from "../acp/plan-gate";
import { appendPlanEntry, planRestoreSource, truncateResolvedAfter, countsAsUserBubble, decideRestoreState, isInterjectionText } from "../acp/plan-restore";
import {
  planReviewFileName,
...
import type {
  ...
  GrokSessionsListOptions,
  RemoteBrowserPreferences,
  RemoteRequester,
  RemoteResumeTarget,
  RemoteVoiceEntry,
...
import { withoutArchiveFields } from "../projects/project-discovery";
import { RemoteUplink } from "../remote/remote-uplink";
import { RemoteClientState, serializesRemoteSessionTransition } from "../remote/remote-client-state";
import { RemotePcmIngress, acceptRemotePcm } from "../remote/remote-voice";
import { SessionRequestState } from "../session/session-request-state";
import { allowFromRemote, capabilitiesForRemote, allowRemoteRepoTarget, bracketRemoteSnapshot, mayDeliverRemoteHostMsg, remoteRequiresBoundSession, repoScopeFor, repoSessionsMessageForRemote, sessionCwdBelongsToRepo, sessionForRequest, shouldAdoptDeskSession, transformHostMsgForRemote, type MediaInlineDeps, type MsgOrigin, type RemoteTier } from "../remote/remote-policy";
import {
...
} from "../remote/remote-files";
import {
  isCloudEnvironment,
  CLOUD_ENVIRONMENT_ENV, buildLinkStartBody, deviceDisplayName, httpBaseFromRelayUrl, parseRelayFrame, RELAY_DEVICE_TOKEN_SECRET, resolveRelayUrl } from "../remote/remote-frames";
import { KeepAwake, shouldKeepAwake } from "../remote/keep-awake";

import { thumbnailImage, thumbnailMime } from "../composer/image-thumbnail";
import { historyImagePreviews } from "../composer/image-history";
import {
  SessionListEntry,
...
import { isExecutableOpenTarget, revalidateOpenFileForUse } from "../desktop/policy/desktop-policy";
import {
  describeFfmpegProblem,
  ffmpegInstallHint,
  resolveConfiguredFfmpeg,
  type FfmpegResolution,
} from "../voice/ffmpeg-locate";
import {
...
import { MCP_GLOBAL_SCOPE_WARNING, mergeMcpNotification, parseMcpListResponse, mcpSettingsServersForCwd, type McpServerView } from "../mcp/mcp";
import {
  MCP_CONNECTORS_KEY,
...
export class GrokSidebar {
  ...
  // remote turn. `grok.remote.keepAwake` is the opt-out. See src/remote/keep-awake.ts.
...
Only continue if you trust this code.`,
  ...
  private async steerSend(
    ...
    const steerText = queuedSendsText(contributions) || authored;
    const hostPlan = applyHostPlanSlashToItems(steerText, contributions);
    if (hostPlan.matched) {
      await this.setMode("plan", session, requester);
      if (gen !== session.gen || session.client !== client) {
        putBackOnComposer();
        return;
      }
      const hasChips = contributions.some((item) => item.chips.some((chip) => !chip.hidden));
      if (!hostPlan.text && !hasChips) return;
    }
   ...
   *
   * A name, never a path — see src/projects/project-create.ts for why that is the whole
...
${many ? `${working.length} conversations are` : "A conversation is"} still working. ` +
  ...
  private async startSessionBody(
    ...
    client.on("commandsUpdate", (cmds) => {
      if (gen !== session.gen) return;
      this.emit(session, { type: "commandsUpdate", commands: mergeHostSlashCommands(cmds) });
      if (session.provider === "grok") {
  ...
  private async handleSend(
    ...
    const hostPlan = parseHostPlanSlash(text);
    if (hostPlan) {
      await this.setMode("plan", session);
      if (gen !== session.gen) return;
      const queuedHasChips = (queuedSendCommit?.items ?? []).some((item) =>
        item.chips.some((chip) => !chip.hidden),
      );
      const composerHasChips = !queuedSendCommit && !bare
        && explicitVisibleChips(session.chips).length > 0;
      if (!hostPlan.remainder && !queuedHasChips && !composerHasChips) {
        if (queuedSendCommit) {
    ...
    if (hostPlan && contributions?.length) {
      applyHostPlanSlashToItems(contributions[0].text, contributions);
    }
  ...
  private postInitialState(): void {
    ...
    this.post({ type: "commandsUpdate", commands: mergeHostSlashCommands([]) });
    this.postProviderState();
    void this.refreshGithubState();
    this.postMcpConnectors();
...
<body class="settings-page">
  ...
  private getHtml(webview: HostWebview): string {
    return renderChatHtml({
      ...
      canSwitchWorkspaceFolder: this.host.canSwitchWorkspaceFolder,
      thinkingHidden: !this.showThinking(),
      chatFontScale: this.chatFontScale(),
    });
  }
}
